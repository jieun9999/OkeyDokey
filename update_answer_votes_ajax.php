<?php
  ...
  if (isset($_POST['answerId']) && isset($_POST['voteType'])) {
  ...
  // 2. UPDATE 쿼리 작성
  if($voteType === "up"){
    $sql = "UPDATE answers SET  answerVotingCounts =  answerVotingCounts +1 WHERE answerId =  $answerId";
  ...
  if($result){
    ...
    $updatedVoteCount = fetchUpdateVoteCount($answerId);
...
// 특정 $answerId에 대해 업데이트된 투표 수를 가져오는 함수
function fetchUpdateVoteCount($answerId){

    global $connect;
    $sql2 = "SELECT answerVotingCounts FROM answers WHERE answerId =  $answerId";
    $result2 = $connect -> query($sql2);

    if($result2 && $row = $result2->fetch_assoc()){
        return $row['answerVotingCounts'];
    }else {
