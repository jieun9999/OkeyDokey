<?php
    ...
    $delCommentId = isset($_POST['deleteCommentId']) ? $_POST['deleteCommentId'] : null;
    echo $delCommentId; // 콘솔에 출력해서 확인

    if ($delCommentId === null) {
        echo '<script>alert("코멘트 ID를 찾을 수 없습니다.");</script>';
   ...
   try{
    ...
    //매개변수 바인딩 및 실행
    $stmt->bindParam(':delCommentId', $delCommentId,  PDO::PARAM_INT);
    $stmt->execute();

    // 삭제 성공 여부에 따른 처리
    if($stmt -> rowCount() > 0){
        echo '<script>alert("코멘트가 삭제되었습니다.");</script>';
        echo '<script>history.back();</script>';
    }else{
        echo '<script>alert("코멘트 삭제에 실패하였습니다");</script>';
        echo '<script>history.back();</script>';
